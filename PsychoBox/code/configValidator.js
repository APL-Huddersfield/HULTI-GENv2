...
//
// ## When a new property is extracted
// 1. Check it has a 'type'
...
function validate(testName, schemaName) {
    // Check if both dictionaries exist
    var x = new Dict;
    var names = x.getnames();

    if (!dictExists(testName)) {
        error("No such test dictionary named " + testName + "\n");
        return;
    }
